...
class Api {
  ...
  async getChunkOfWords(group: string, page: string) {
    let result;
    try {
      const rawResponse = (await this.wordsMethods.getChunkOfWords(
        group,
        page
      )) as Response;
  ...
  async createNewUser(requestBody: User) {
    let result;
    try {
      const rawResponse = (await this.usersMethods.createNewUser(
        requestBody
      )) as Response;
  ...
  async getUser(id: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersMethods.getUser(
        id,
        token
      )) as Response;
  ...
  async updateUser(id: string, token: string, requestBody: UpdateUser) {
    let result;
    try {
      const rawResponse = (await this.usersMethods.updateUser(
        id,
        token,
        requestBody
      )) as Response;
  ...
  async deleteUser(id: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersMethods.deleteUser(
        id,
        token
      )) as Response;
      result = await this.checkResponse(rawResponse);
  ...
  async getNewUserTokens(id: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersMethods.getNewUserTokens(
        id,
        token
      )) as Response;
  ...
  async getAllUserWords(id: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersWordsMethods.getAllUserWords(
        id,
        token
      )) as Response;
      result = await this.checkResponse(rawResponse);
  ...
  async createWord(
    ...
    let result;
    try {
      const rawResponse = (await this.usersWordsMethods.createWord(
        id,
        wordID,
        token,
        requestBody
  ...
  async getWord(id: string, wordID: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersWordsMethods.getWord(
        id,
        wordID,
        token
      )) as Response;
  ...
  async updateWord(
    ...
    wordID: string,
    token: string,
    requestBody: Partial<UserWord>
  ) {
    let result;
    try {
      const rawResponse = (await this.usersWordsMethods.updateWord(
        id,
        wordID,
        token,
        requestBody
  ...
  async deleteWord(id: string, wordID: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersWordsMethods.deleteWord(
        id,
        wordID,
        token
      )) as Response;
  ...
  async getAllUserAggregatedWords(
    ...
    try {
      const rawResponse =
        (await this.usersAggregatedWordsMethods.getAllUserAggregatedWords(
          id,
          token,
          group,
          page,
  ...
  async getAggregatedWord(id: string, wordID: string, token: string) {
    ...
    try {
      const rawResponse =
        (await this.usersAggregatedWordsMethods.getAggregatedWord(
          id,
          wordID,
          token
        )) as Response;
  ...
  async getStatistics(id: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersStatisticMethods.getStatistics(
        id,
        token
      )) as Response;
  ...
  async upsertStatistics(id: string, token: string, requestBody: Statistic) {
    let result;
    try {
      const rawResponse = (await this.usersStatisticMethods.upsertStatistics(
        id,
        token,
        requestBody
      )) as Response;
  ...
  async getSettings(id: string, token: string) {
    let result;
    try {
      const rawResponse = (await this.usersSettingMethods.getSettings(
        id,
        token
      )) as Response;
  ...
  async upsertSettings(id: string, token: string, requestBody: Setting) {
    let result;
    try {
      const rawResponse = (await this.usersSettingMethods.upsertSettings(
        id,
        token,
        requestBody
      )) as Response;
  ...
  async signIn(requestBody: UpdateUser) {
    let result;
    try {
      const rawResponse = (await this.signInMethods.signIn(
        requestBody
      )) as Response;
  ...
  returnsErrorMessage(error: Error) {
    return {
      isSuccess: false,
      data: { errorMessage: `${error.name}: ${error.message}` },
    };
  }
  ...
  async checkResponse(response: Response, method?: string) {
    switch (response.status) {
      ...
      case 204:
        let message =
          "Don't panic. Everything is under control. Continue to work. Big brother is watching you.";
      ...
      case 402:
        return {
          isSuccess: false,
          data: { errorMessage: "Access token is missing or invalid" },
        };
      case 403:
        switch (method) {
          ...
          case "signIn":
            return {
              isSuccess: false,
              data: { errorMessage: "Incorrect e-mail or password" },
            };
          default:
        }
        break;
      case 404:
        switch (method) {
          ...
          case "getAggregatedWord":
            return {
              isSuccess: false,
              data: { errorMessage: "User's word not found" },
            };
          case "getUser":
            return {
              isSuccess: false,
          ...
          case "getStatistics":
            return {
              isSuccess: false,
              data: { errorMessage: "Statistics not found" },
            };
          case "getSettings":
            return {
              isSuccess: false,
      ...
      case 417:
        return {
          isSuccess: false,
          data: { errorMessage: "User with this e-mail exists" },
        };
      case 422:
        switch (method) {
          case "createNewUser":
          ...
          case "updateWord":
            return {
              isSuccess: false,
              data: { errorMessage: "Unprocessable Entity" },
            };
          default:
            return { isSuccess: false, data: { errorMessage: "Not found" } };
        }
