...
const SprintStructure = (words) => {
  ...
  let timeLeft = 30;
  const realAnswer = (words[wordNum].wordTranslate === translateAnswer);

  useEffect(() => {
    const keyPress = (event) => {
        if (event.keyCode === 39) { //right
          handleClick(realAnswer, true, count, setCount, maxSeries, setMaxSeries, words, wordNum, setWordNum);}
        else if (event.keyCode === 37) { //left
          handleClick(realAnswer, false, count, setCount, maxSeries, setMaxSeries, words, wordNum, setWordNum);
        }
      }
...
}, [count, maxSeries, realAnswer, wordNum, words]);
  ...
  <div>
    ...
    <div>  
      ...
      {ShowButtons(words, wordNum, setWordNum, count, setCount, maxSeries, setMaxSeries, realAnswer)}
    </div>
  </div>
  )
}
...
const SprintButtons = () => {
  return (
    <div>
      ...
      <svg xml ns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-volume-up" viewBox="0 0 16 16">
